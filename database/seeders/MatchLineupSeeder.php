...
class MatchLineupSeeder extends Seeder
{
    ...
    public function createMatchLineups($playerEditionIds, $soccerMatchId, $statusLineupId){
        ...
        foreach ($playerEditionIds as $playerEditionId) {
            ...
            $matchLineupScore = MatchLineupScore::where('match_lineup_id', $matchLineupId)->first();

            if($matchLineupScore == null){
                $newMatchGameLineupScore = MatchLineupScore::create([
                    'score' => 6.0,
                    'match_lineup_id' => $matchLineupId
                ]);
            }
        }
